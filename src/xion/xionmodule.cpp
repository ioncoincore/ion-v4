...
bool XIONModule::createInput(CTxIn &in, CZerocoinMint mint, uint256 hashTxOut){
    ...
    if (!key.Sign(hashTxOut, vchSig))
        throw std::runtime_error("XIONModule failed to sign hashTxOut\n");
...
PublicCoinSpend XIONModule::parseCoinSpend(const CTxIn &in, const CTransaction& tx){
...
bool XIONModule::validateInput(const CTxIn &in, const CTxOut &prevOut, const CTransaction& tx, PublicCoinSpend& ret){
