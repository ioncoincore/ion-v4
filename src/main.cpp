...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : xION v1 serial spend not spendable, serial %s, tx %s\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!XIONModule::validateInput(txin, prevOut, tx, ret)){
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that xION mints (if included) are not already known
        ...
        // No relaying of zero fee transactions.
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST  && nHeight > Params().MidasStartHeight()
            && block.GetBlockTime() <= pindexPrev->GetMedianTimePast())
        ...
        if (isBlockFromFork) {
            ...
            if(hasXIONInputs){
                for (const CTxIn& xIonInput : xIONInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(xIonInput);
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        bool fMissingSporks = !pSporkDB->SporkExists(SPORK_8_NEW_PROTOCOL_ENFORCEMENT) &&
                ...
                !pSporkDB->SporkExists(SPORK_10_TOKENGROUP_MAINTENANCE_MODE) &&
                !pSporkDB->SporkExists(SPORK_11_NEW_PROTOCOL_ENFORCEMENT_2);
...
int ActiveProtocol()
{
    // SPORK_8 was used for 95704 (v3.1.0+), commented out now.
    //if (IsSporkActive(SPORK_8_NEW_PROTOCOL_ENFORCEMENT))
    //        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_11 is used for 95705 (v3.3+)
    if (IsSporkActive(SPORK_11_NEW_PROTOCOL_ENFORCEMENT_2))
            return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;
