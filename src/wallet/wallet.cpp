...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use xION in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = xionTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = xionTracker->ListMints(true, true, true);
        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if ((GetBoolArg("-xionstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_9_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        std::set<CMintMeta> setMints = xionTracker->ListMints(true, true, fUpdate);
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
                receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), XION_TXMINT_GENERAL);
                return false;
            } else if (mapBlockIndex.count(hashBlock) < 1) {
                // check that this mint made it into the blockchain
                receipt.SetStatus(_("Mint did not make it into blockchain"), XION_TXMINT_GENERAL);
                return false;
            }
            ...
            if (outputIndex == -1) {
                receipt.SetStatus(_("Pubcoin not found in mint tx"), XION_TXMINT_GENERAL);
                return false;
            }

            mint.SetOutputIndex(outputIndex);
            CTxIn in;
            if(!XIONModule::createInput(in, mint, hashTxOut)){
                receipt.SetStatus(_("Cannot create public spend input"), XION_TXMINT_GENERAL);
                return false;
            }
            vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    if (vSelectedMints.empty()) {
        ...
        setMints = xionTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
...
std::string CWallet::GetUniqueWalletBackupName(bool fxionAuto) const
{
...
void CWallet::XIonBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autoxionbackup-%d.dat", i + 1);
    ...
    if(!GetArg("-xionbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-xionbackuppath", ""));
...
{
    // We don't even need to worry about this code.. no xION.
    /*
