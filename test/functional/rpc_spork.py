...
class ION_RPCSporkTest(BitcoinTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 2
        self.extra_args = [['-staking=1']] * self.num_nodes
        self.extra_args[0].append('-sporkkey=93QPD8M8SrVb4yL3E679sCGztzy1NRWYH3fs2wJQr2LMKnppFCJ')
    ...
    def run_test(self):
        ...
        assert(not active["SPORK_4_MASTERNODE_PAYMENT_ENFORCEMENT"])
        # activate SPORK 4
        new_value = 1563253447
        res = self.nodes[0].spork("SPORK_4_MASTERNODE_PAYMENT_ENFORCEMENT", new_value)
        assert(res == "success")
        sleep(1)
        self.sync_all()
        sporks = self.nodes[1].spork("show")
        self.printDict(sporks)
        assert(sporks["SPORK_4_MASTERNODE_PAYMENT_ENFORCEMENT"] == new_value)
        active = self.nodes[0].spork("active")
        assert (active["SPORK_4_MASTERNODE_PAYMENT_ENFORCEMENT"])



if __name__ == '__main__':
    ION_RPCSporkTest().main()
